...
void ___logger_pars(___logger *lvg, const char *format, ___logger_token_mas **mas_opt_f, uintmax_t *num_token)
{
    ___logger_token_mas *mas_opt = malloc(sizeof(___logger_token_mas));
        ...
        for (uintmax_t i = 0; i < strlen(format); i++) {
            ...
            if (format[i] == ___logger_token_list[LEF_TOK].name[0] && isalpha(format[i + 1])) {
                ...
                if (format[i] == ___logger_token_list[LEF_SPE].name[0]
                    ...
                    mas_opt[j].spe      = calloc(1, sizeof(___logger_token_specifiers));
                    mas_opt[j].spe->beg = i;
                    do {
                        mas_opt[j].spe->end = i;
                        i++;
                ...
                do {
                    mas_opt[j].end = i;
                    i++;
                } while (!(format[i] == ___logger_token_list[LEF_TOK].name[0] && isalpha(format[i + 1]))
                         && (format[i] != '\0'));
                i--;
            }
            j++;
    ...
    { /*Парсинг спецификаторов*/
        for (uintmax_t i = 0; i < *num_token; i++) {
            if (mas_opt[i].spe != NULL) {
                ...
                if (format[mas_opt[i].spe->beg + 1] == ___logger_token_list[LEF_HID].name[0]) {
                    beg++;
                    mas_opt[i].spe->lfs = LFS_LEF;
                }
                if (format[mas_opt[i].spe->end] == ___logger_token_list[LEF_HID].name[0]
                    && mas_opt[i].spe->lfs != LFS_LEF) {
                    end--;
