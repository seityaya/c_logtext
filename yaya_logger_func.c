        ...
        for (uintmax_t i = 0; i < *num_token; i++) {
            if (mas_opt[i].spe != NULL) {
                ...
                if (format[mas_opt[i].spe->beg + 1] == logger_format_list[LEF_HID].name[0]) {
                    beg++;
                    mas_opt[i].spe->fes = LFS_LEF;
                }
                if (format[mas_opt[i].spe->end] == logger_format_list[LEF_HID].name[0] && mas_opt[i].spe->fes != LFS_LEF) {
                    end--;
                    mas_opt[i].spe->fes = LFS_RIG;
                }
                if (format[beg] == '0') {
                    beg++;
                    mas_opt[i].spe->fes = LFS_NUL;
                }
                ...
                if (format[beg] == logger_format_list[LEF_SEP].name[0]) {
                    if (mas_opt[i].spe->fes != LFS_NUL && mas_opt[i].spe->fes != LFS_LEF) {
                        mas_opt[i].spe->fes = LFS_RIG;
                    }
                    sscanf(buff, ".%ld", &mas_opt[i].spe->rig);
                } else {
                    if (mas_opt[i].spe->fes != LFS_NUL && mas_opt[i].spe->fes != LFS_RIG) {
                        mas_opt[i].spe->fes = LFS_LEF;
                    }
...
void format_build_str(logger_format_mas *mas_opt, const char *buff_orig) {
    ...
    if (mas_opt->spe != NULL) {
        ...
        } else if (mas_opt->spe->fes == LFS_RIG) {
...
void format_build_num(logger_format_mas *mas_opt, uintmax_t *buff_orig) {
    if (mas_opt->spe != NULL) {
        if (mas_opt->spe->fes == LFS_NUL) {
            lvg->out_offset += sprintf(&lvg->out_buff[lvg->out_offset], "%0*ld", (int) mas_opt->spe->lef, (uintmax_t) *buff_orig);
        } else if (mas_opt->spe->fes == LFS_LEF) {
            lvg->out_offset += sprintf(&lvg->out_buff[lvg->out_offset], "%*ld", (int) mas_opt->spe->lef, (uintmax_t) *buff_orig);
        } else if (mas_opt->spe->fes == LFS_RIG) {
            lvg->out_offset += sprintf(&lvg->out_buff[lvg->out_offset], "%-*ld", (int) mas_opt->spe->rig, (uintmax_t) *buff_orig);
        }
    } else {
        lvg->out_offset += sprintf(&lvg->out_buff[lvg->out_offset], "%ld", (uintmax_t) *buff_orig);
    }
}
